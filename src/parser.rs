...
impl Parser {
    ...
    //           ( "else" statement )? ;
    fn if_statement(&mut self) -> Result<Option<Stmt>, LoxError> {
    ...
    fn return_statement(&mut self) -> Result<Option<Stmt>, LoxError> {
    ...
    // block -> "{" declaration* "}" ;
    fn block(&mut self) -> Result<Vec<Option<Stmt>>, LoxError> {
        let mut statements: Vec<Option<Stmt>> = vec![];
